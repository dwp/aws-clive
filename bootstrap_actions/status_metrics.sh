...
(
    ...
    function log_wrapper_message() {
        log_aws_clive_message "$${1}" "status_metrics.sh" "$${PID}" "$${@:2}" "Running as: ,$USER"
    }
  ...
  push_metric() {
    ...
    cat << EOF | curl --data-binary @- "http://${clive_pushgateway_hostname}:9091/metrics/job/clive"
                clive_status{component="Clive",snapshot_type="$SNAPSHOT_TYPE", export_date="$EXPORT_DATE", cluster_id="$CLUSTER_ID",correlation_id="$CORRELATION_ID"} $1
EOF
...
) >> /var/log/aws-clive/status_metrics.log 2>&1
