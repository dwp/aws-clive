...
resource "aws_lambda_function" "aws_clive_emr_launcher" {
  ...
  environment {
    variables = {
      ...
      EMR_LAUNCHER_LOG_LEVEL        = local.aws_clive_log_level[local.environment]
    }
  }
}
...
resource "aws_sns_topic_subscription" "clive_trigger_sns" {
  topic_arn = aws_sns_topic.clive_cw_trigger_sns.arn
  protocol  = "lambda"
  endpoint  = aws_lambda_function.aws_clive_emr_launcher.arn
}

resource "aws_lambda_permission" "clive_emr_launcher_subscription" {
  statement_id  = "CWTriggercliveSNS"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.aws_clive_emr_launcher.function_name
  principal     = "sns.amazonaws.com"
  source_arn    = aws_sns_topic.clive_cw_trigger_sns.arn
}
